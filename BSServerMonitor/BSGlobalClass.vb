...
#End Region
        Public Sub GetSettings(ByRef LastSucBackup As String, ByRef AlertOnBackUp As Boolean, ByRef TrackHistoryDays As Integer, ByRef TrackHistory As Boolean, ByRef AutoBackup As Boolean, ByRef UOIMG As Boolean, ByRef UseIPer As Boolean, Optional ByRef ConfigSort As String = "All")
            ...
            'Dim strValue As String = DefaultRegPath & "\Settings"
            'Try
            'TrackHistoryDays = CInt(GetRegSubKeyValue(strValue, "TrackHistoryDays", 30))
            ...
            'AlertOnBackUp = CBool(GetRegSubKeyValue(strValue, "AlertOnBackUp", "False"))
            'AutoBackup = CBool(GetRegSubKeyValue(strValue, "BackupOnExit", "False"))
            'UOIMG = CBool(GetRegSubKeyValue(strValue, "UseOrgImage", "True"))
            'LOADERTYPE_SHOTGUN = CBool(GetRegSubKeyValue(strValue, "LOADERTYPE_SHOTGUN", "True"))
    ...
    Public Class BSDatabase
        ...
        Public Function DataExists(ByVal SQL As String) As Boolean
            ...
            Catch ex As Exception
                Dim sMsg As String = " - ERROR - " & Err.Number & " - " & ex.Message.ToString
                Call CatchError(sMsg, "modGlobal.DataExists")
            End Try
            Return bAns
        End Function
        Public Sub CloseDB()
            If Conn.State <> ConnectionState.Connecting Or Conn.State <> ConnectionState.Broken Then Conn.Close()
            Conn = Nothing
        End Sub
    End Class
    Public Class BSAutoComplete
        Private Function MainCollection(ByVal strColumn As String, ByVal strTable As String) As AutoCompleteStringCollection
            ...
            Catch ex As Exception
                Dim sMsg As String = " - ERROR - " & Err.Number & " - " & ex.Message.ToString
                Call CatchError(sMsg, "BSAutoComplete.MainCollection")
            End Try
            Return iCol
        End Function
        Public Function Server_Type_Exists() As AutoCompleteStringCollection
            Return MainCollection("Cat", "list_servers_types")
        End Function
    End Class
